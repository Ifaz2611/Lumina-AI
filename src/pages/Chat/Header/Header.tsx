...
function Header() {
    ...
    return (
        <div className='header'>
            ...
            >
                <Icon icon={isMenuOpen ? "mdi:close" : "mdi:menu"} height={28} />
            </button>

            {/* Mobile Menu */}
            {isMenuOpen && (
                <div id="mobile-menu" className="mobile-menu" ref={menuRef} role="dialog" aria-modal="true" aria-label="Menu">
                    <ModelSelector />
                    <IconButton
                        icon="mdi:github"
                        onClick={() => { openGithub(); setIsMenuOpen(false); }}
                        ariaLabel="GitHub – open repository"
                    />
                    <IconButton
                        icon="mdi:trashcan-outline"
                        onClick={() => { dispatch(clearChat()); setIsMenuOpen(false); }}
                        ariaLabel="Clear chat history"
                    />
                    <ThemeToggle />
                    <Logout />
